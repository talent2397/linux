...
static void send_wifi_status(bool connected)
{
    if (!g_ipc_ep_ui)
    {
    ...
    {
        json j;
        j["wifi"] = connected ? "1" : "0";
        std::string wifiString = j.dump();
        g_ipc_ep_ui->send(g_ipc_ep_ui, wifiString.data(), wifiString.size());
        std::cout << "Send WiFi status: " << wifiString << std::endl;
    }
    catch (const std::exception &e)
    {
        std::cerr << "Error creating WiFi status JSON: " << e.what() << std::endl;
    }
}
...
static void send_emotion(const std::string &text)
{
    if (!g_ipc_ep_ui)
    {
        std::cerr << "Error: g_ipc_ep_ui is nullptr" << std::endl;
    ...
    try
    {
        ...
        else if (j["type"] == "llm")
        {
            ...
            */
            ...
            send_emotion(emotion.get<std::string>());
        }
        else if (j["type"] == "iot")
        {
        }
...
bool check_network()
{
    return system("ping -c 1 -W 2 8.8.8.8 > /dev/null 2>&1") == 0;
}

// 同步时间
bool sync_time()
{
    return system("ntpdate -s pool.ntp.org > /dev/null 2>&1") == 0;
}
...
int main(int argc, char **argv)
{
    ...
    post_stream << R"(
        {
            ...
            "board": {
                "type": "xiaozhi_linux_board", 
                "name": "xiaozhi_kelpbar_board" 
            }
        }
    )";
    ...
    while (1)
    {
        ...
        if (0 != active_device(&http_data, active_code))
        {
            if (active_code[0])
            {
            ...
            printf("设备激活失败，5秒后重试...\n");
            send_stt("设备激活失败，网络时间未同步，点击主页时钟图标同步");
            sleep(5);
        }
        else
        {

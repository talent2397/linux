...
int open_play(const char *device, unsigned int sample_rate, unsigned int channels, snd_pcm_format_t format, unsigned int *actual_sample_rate, unsigned int *actual_channels, snd_pcm_format_t *actual_format, snd_pcm_t **pcm_handle) {
    ...
    int dir = 0;
    // 1. 强制设置合理的周期大小（目标：10-30ms延迟）
    snd_pcm_uframes_t period_frames = 256; // 16ms @16kHz
    rc = snd_pcm_hw_params_set_period_size_near(*pcm_handle, hw_params, &period_frames, &dir);
    if (rc < 0) {
        fprintf(stderr, "无法设置播放周期大小: %s，尝试更小值\n", snd_strerror(rc));
        period_frames = 128;
        rc = snd_pcm_hw_params_set_period_size_near(*pcm_handle, hw_params, &period_frames, &dir);
    }
    ...
    rc |= snd_pcm_hw_params_set_buffer_size_near(*pcm_handle, hw_params, &buffer_frames);
    if (rc < 0) {
        fprintf(stderr, "无法设置播放缓冲区大小: %s\n", snd_strerror(rc));
        // 不直接返回，尝试继续
    }
    ...
    // 4. 重新获取并打印实际生效的参数（用于验证）
    snd_pcm_uframes_t actual_period_frames;
    snd_pcm_hw_params_get_period_size(hw_params, &actual_period_frames, &dir);
    printf("优化后的播放参数 -> 周期: %lu帧(%.1fms)\n",
        (unsigned long)actual_period_frames,
        (double)actual_period_frames / sample_rate * 1000);

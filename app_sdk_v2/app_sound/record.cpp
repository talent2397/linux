...
int open_record(const char *device, unsigned int sample_rate, unsigned int channels, snd_pcm_format_t format, unsigned int *actual_sample_rate, unsigned int *actual_channels, snd_pcm_format_t *actual_format, snd_pcm_t **pcm_handle) {
    ...
    // 设置合理的周期大小（对于16kHz，256-1024是合理范围）
    snd_pcm_uframes_t period_size = 512; // 或 256, 1024
    int dir = 0;
    rc |= snd_pcm_hw_params_set_period_size_near(*pcm_handle, hw_params, &period_size, &dir);
    printf("DEBUG: Set period size to %lu frames (dir=%d)\n", (unsigned long)period_size, dir);

    // 同时设置缓冲区大小为4个周期
    snd_pcm_uframes_t buffer_size = period_size * 4;
    rc |= snd_pcm_hw_params_set_buffer_size_near(*pcm_handle, hw_params, &buffer_size);
...
void* record_audio_thread(void* arg) {
    ...
    for (int i = 0; i < actual_channels; i++) {
        ...
        if (!channel_buffers[i]) {
            fprintf(stderr, "Failed to allocate channel buffer %d\n", i);
            for (int j = 0; j < i; j++) free(channel_buffers[j]);
            free(buffer);
            snd_pcm_close(pcm_handle);
            return NULL;
        }

...
p_ipc_endpoint_t ipc_endpoint_create_udp(int port_local, int port_remote, transfer_callback_t cb, void *user_data)
{
    ...
    // 在 ipc_endpoint_create_udp 函数中，bind 之前：
    int reuse = 1;

    // 设置 SO_REUSEADDR（必须的）
    setsockopt(fd_recv, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse));

    // 设置 SO_LINGER 选项，确保套接字快速关闭
    struct linger linger_opt;
    linger_opt.l_onoff = 1;   // 启用 linger
    linger_opt.l_linger = 0;  // 立即关闭，不等待
    setsockopt(fd_recv, SOL_SOCKET, SO_LINGER, &linger_opt, sizeof(linger_opt));
